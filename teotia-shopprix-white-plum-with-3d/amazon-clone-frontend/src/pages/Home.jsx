    ...
    const loadAllProducts = async () => {
      try {
        ...
        if (!cancelled) {
          ...
          // Show only 1–2 unique products. The starting position shifts every day,
          // so the highlighted products rotate daily without duplicating an item.
          ...
          if (uniqueProducts.length <= 2) {
            setTodayHighlights(uniqueProducts)
          } else {
            const dayNumber = Math.floor(Date.now() / 86400000)
            const start = dayNumber % uniqueProducts.length
            setTodayHighlights([
              uniqueProducts[start],
              uniqueProducts[(start + 1) % uniqueProducts.length],
            ])
          }
        }
      } catch {
