    ...
    const loadAllProducts = async () => {
      try {
        ...
        if (!cancelled) {
          ...
          // Pick 1–2 products deterministically from the day, so the highlight changes daily.
          const dayNumber = Math.floor(Date.now() / 86400000)
          const shuffled = [...products].sort((a, b) => {
            const hashA = ((dayNumber + String(a._id || a.id || a.name || '').length * 31) * 9301 + 49297) % 233280
            const hashB = ((dayNumber + String(b._id || b.id || b.name || '').length * 31) * 9301 + 49297) % 233280
            return hashA - hashB
          })
          setTodayHighlights(shuffled.slice(0, Math.min(2, shuffled.length)))
        }
      } catch {
  ...
  return (
    <main className="bg-luxe-bg min-h-screen">
      ...
      <ProductCarousel
        ...
        subtitle="A fresh pick every day"
        products={todayHighlights}
        speed={0}
      />
