    ...
    <main className="bg-luxe-bg min-h-screen px-4 py-8 sm:py-10 font-sans">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-3xl border border-[#E9E4EF] shadow-sm p-6 sm:p-8">
          ...
          {loading ? (
            <div className="min-h-[300px] flex items-center justify-center text-sm text-slate-500">Loading...</div>
          ) : items.length ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-7">
              {items.map((item) => (
                <article key={item._id} className="overflow-hidden rounded-2xl border border-[#E9E4EF] bg-white shadow-sm">
                  {item.type === 'video' ? <video src={item.url} controls className="w-full aspect-video object-contain bg-slate-100" /> : <img src={item.url} alt={item.title || active.label} className="w-full aspect-video object-contain bg-slate-100" />}
                  <div className="p-4">
                    {item.title && <h2 className="font-black text-slate-900">{item.title}</h2>}
