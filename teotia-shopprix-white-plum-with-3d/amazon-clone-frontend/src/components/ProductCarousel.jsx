...
export default function ProductCarousel({ title, subtitle, products = [], speed = 28, reverse = false, loop = true }) {
  ...
  useEffect(() => {
    ...
    if (!el || !loop || items.length < 2) return
  ...
  }, [items.length, reverse, speed, loop])

  useEffect(() => {
    const el = trackRef.current
    if (!el || !loop || items.length < 2) return
    if (reverse) el.scrollLeft = el.scrollWidth / 2
  }, [items.length, reverse, loop])
  ...
  // Some sections (such as Today's Highlights) should show each selected product only once.
  const loopItems = loop && items.length > 1 ? [...items, ...items] : items
