  ...
  return (
    <section className="bg-white">
      ...
      <div className="relative w-full overflow-hidden bg-white">
        {slide.type === 'video' ? (
          <video key={slide.url} src={slide.url} autoPlay muted loop playsInline className="w-full h-[330px] sm:h-[430px] lg:h-[510px] object-cover" />
        ) : (
          <img key={slide.url} src={slide.url} alt="Teotia Shopprix offer banner" loading="eager" className="w-full h-[330px] sm:h-[430px] lg:h-[510px] object-cover" />
        )}
        ...
        <div className="hidden md:block absolute right-[6%] top-1/2 -translate-y-1/2 w-[220px] lg:w-[260px] rounded-xl bg-white/95 p-5 lg:p-6 shadow-xl border border-white/80">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#e52b59]">Special Delivery Offer</p>
        ...
        {slides.length > 1 && (
          <>
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex gap-2">
              {slides.map((_, i) => <button key={i} onClick={() => goTo(i)} aria-label={`Banner ${i + 1}`} className={`h-2 rounded-full transition-all ${i === index ? 'bg-[#e52b59] w-7' : 'bg-white/90 w-2.5'}`} />)}
            </div>
          </>
        )}
      </div>
      ...
      {/* Trust strip */}
      <div className="border-b border-[#eee3e7] bg-white">
        <div className="max-w-[1440px] mx-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 divide-x divide-[#eee3e7]">
          {trustItems.map(({ icon: Icon, title, text }) => (
            <div key={title} className="flex items-center gap-3 px-4 lg:px-7 py-4 lg:py-5">
              <span className="w-10 h-10 shrink-0 rounded-full bg-[#fff5f7] border border-[#f3d7df] flex items-center justify-center text-[#e52b59]"><Icon size={20} /></span>
