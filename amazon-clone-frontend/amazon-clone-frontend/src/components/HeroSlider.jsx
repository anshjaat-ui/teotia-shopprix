...
export default function HeroSlider() {
  ...
  return (
    <div className="relative overflow-hidden bg-luxe-bg h-[320px] md:h-[440px]">
      ...
      {slides.length > 1 && (
        <>
          <button onClick={prev} className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/50 flex items-center justify-center text-gold hover:bg-black/70">
            <ChevronLeft size={20} />
          </button>
          <button onClick={next} className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/50 flex items-center justify-center text-gold hover:bg-black/70">
