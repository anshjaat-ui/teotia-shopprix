            ...
            return (
              <div key={s._id} className="bg-luxe-panel border border-gold/20 rounded-lg p-5">
                ...
                <div className="space-y-1 text-sm text-gray-300 mb-4">
                  <div className="flex justify-between"><span>One-time Deposit:</span><span className="text-white font-medium">₹{(s.depositAmount || 0).toLocaleString('en-IN')}</span></div>
                  <div className="flex justify-between"><span>Monthly Coupon:</span><span className="text-white">₹{s.monthlyPayout || 0} × {s.durationMonths - 1} months</span></div>
                  <div className="flex justify-between"><span>Last Month Coupon:</span><span className="text-white">₹{s.finalPayout || 0}</span></div>
                ...
                {!enrollment ? (
                  ...
                  <div className="border-t border-gold/10 pt-3">
                    ...
                    {showResult && lastRedeemed.coupons.length > 0 && (
                      <div className="bg-black/30 border border-gold/20 rounded-md p-2 mb-3 space-y-1">
                        ...
                        {lastRedeemed.coupons.map((c) => (
                          <div key={c.code} className="flex items-center justify-between text-xs">
                            ...
                            <button onClick={() => copyCode(c.code)}><Copy size={11} className="text-gray-400" /></button>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {enrollment.coupons.some((c) => c.redeemed) && (
                      <details className="mb-3">
                        <summary className="text-[11px] text-gray-500 cursor-pointer">Past coupons</summary>
                        <div className="mt-1 space-y-1">
                          {enrollment.coupons.filter((c) => c.redeemed).map((c) => (
                            <div key={c.monthNumber} className="flex justify-between text-[11px] text-gray-400">
                    ...
                    {enrollment.status === 'completed' ? (
                      ...
                      <button
                        ...
                        className="w-full bg-gold hover:bg-gold-light text-black text-sm font-semibold py-2 rounded-full disabled:opacity-60"
                      >
