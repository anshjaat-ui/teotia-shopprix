    ...
    <div className="bg-luxe-panel border-b border-gold/10">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4 overflow-x-auto">
        ...
        >
          ...
          <span className={`text-[11px] whitespace-nowrap ${!activeSubcategory ? 'text-gold font-medium' : 'text-gray-400'}`}>All</span>
        </div>
        ...
        {subcategories.map((sub) => (
          ...
          >
            ...
            <span className={`text-[11px] whitespace-nowrap ${activeSubcategory === sub.name ? 'text-gold font-medium' : 'text-gray-400'}`}>
              {sub.name}
            </span>
          </div>
        ))}
