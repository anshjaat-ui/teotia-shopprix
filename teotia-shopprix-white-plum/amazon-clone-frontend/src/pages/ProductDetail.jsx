...
function SpinViewer({ images }) {
  ...
  return (
    ...
    >
      ...
      <span className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-gold/60 text-gold text-[10px] px-3 py-1 rounded-full">
        Drag to rotate 360°
      </span>
    </div>
  )
    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((n) => (
        <button key={n} type="button" onClick={() => setRating(n)}>
          <Star size={22} className="text-gold" fill={n <= rating ? '#6D214F' : 'none'} strokeWidth={1.5} />
        </button>
      ))}
    </div>
  )
          ...
          <div className="flex items-center gap-2 mb-3">
            <div className="flex text-gold">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} size={16} fill={i < Math.round(rating) ? '#6D214F' : 'none'} strokeWidth={1} />
              ))}
            </div>
            ...
            {reviews.map((r) => (
              <div key={r._id} className="bg-luxe-panel border border-gold/10 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-1">
                  <div className="flex text-gold">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star key={i} size={12} fill={i < r.rating ? '#6D214F' : 'none'} strokeWidth={1} />
                    ))}
                  </div>
        ...
        <div className="bg-luxe-panel border border-gold/10 rounded-lg p-4 max-w-md">
          ...
          <form onSubmit={handleReviewSubmit} className="space-y-3">
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-slate-50 border border-gold/30 text-slate-900 rounded-md px-3 py-2 text-sm"
            />
            <button
              type="submit"
