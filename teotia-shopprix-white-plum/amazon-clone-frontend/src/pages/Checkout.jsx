          ...
          <div className="bg-luxe-panel border border-gold/20 rounded-lg p-6 text-center">
            {claimed ? (
              <div className="py-8">
                ...
                <p className="text-sm text-gray-400 mt-1">Redirecting to your orders...</p>
              </div>
            ) : (
              <>
                ...
                <div className="flex items-center justify-center gap-2 mb-1">
                  <span className="text-sm text-gray-300">{paymentInfo.upiId}</span>
                ...
                <div className="text-left mt-6 border-t border-gold/10 pt-4">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-gold/30 text-slate-900 rounded-sm px-2 py-1.5 text-sm mb-2"
                  />
                  <button
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4 px-4 py-6">
        <form onSubmit={handlePlaceOrder} className="lg:col-span-2 bg-luxe-panel border border-gold/20 p-5 rounded-lg space-y-3">
          {deliveryStatus === 'checking' && (
            <div className="bg-white/5 border border-gold/20 text-gray-300 text-sm px-3 py-2 rounded flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input name="fullName" required placeholder="Full name" value={address.fullName} onChange={handleChange} className="bg-slate-50 border border-gold/30 text-slate-900 rounded-sm px-2 py-1.5 col-span-2" />
            <input name="phone" required placeholder="Phone number" value={address.phone} onChange={handleChange} className="bg-slate-50 border border-gold/30 text-slate-900 rounded-sm px-2 py-1.5 col-span-2" />
            <input name="line1" required placeholder="Address line 1" value={address.line1} onChange={handleChange} className="bg-slate-50 border border-gold/30 text-slate-900 rounded-sm px-2 py-1.5 col-span-2" />
            <input name="line2" placeholder="Address line 2 (optional)" value={address.line2} onChange={handleChange} className="bg-slate-50 border border-gold/30 text-slate-900 rounded-sm px-2 py-1.5 col-span-2" />
            <input name="city" required placeholder="City" value={address.city} onChange={handleChange} className="bg-slate-50 border border-gold/30 text-slate-900 rounded-sm px-2 py-1.5" />
            <input name="state" required placeholder="State" value={address.state} onChange={handleChange} className="bg-slate-50 border border-gold/30 text-slate-900 rounded-sm px-2 py-1.5" />
            <input name="pincode" required placeholder="Pincode" value={address.pincode} onChange={handleChange} className="bg-slate-50 border border-gold/30 text-slate-900 rounded-sm px-2 py-1.5 col-span-2" />
          </div>
          ...
          <div className="border-t border-gold/10 pt-3">
            ...
            {appliedCoupon ? (
              ...
              <div className="flex gap-2">
                <input value={couponInput} onChange={(e) => setCouponInput(e.target.value.toUpperCase())} placeholder="Enter coupon code" className="flex-1 bg-slate-50 border border-gold/30 text-slate-900 rounded-sm px-2 py-1.5 text-sm" />
                <button type="button" onClick={handleApplyCoupon} disabled={applyingCoupon} className="bg-white/5 hover:bg-white/10 border border-gold/30 text-gold px-4 rounded-sm text-sm disabled:opacity-60">
          ...
          <div className="border-t border-gold/10 pt-3">
            ...
            <div className="space-y-2">
              {settings?.qrUpiEnabled !== false && (
                <label className="flex items-center gap-2 bg-gold/20 border border-gold/20 rounded-md px-3 py-2 cursor-pointer">
                  <input type="radio" name="pm" checked={paymentMethod === 'upi'} onChange={() => setPaymentMethod('upi')} />
                  <span className="text-sm text-gray-200">UPI / QR Code (GPay, PhonePe, Paytm)</span>
                </label>
              )}
