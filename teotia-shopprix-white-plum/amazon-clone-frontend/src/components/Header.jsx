    ...
    <header className="sticky top-0 z-50 font-sans bg-luxe-bg border-b border-gold/30">
      <div className="flex items-center gap-3 px-3 py-3">
        ...
        <Link to="/" className="flex items-center shrink-0">
          <span className="text-lg sm:text-2xl font-bold tracking-wide text-gold whitespace-nowrap drop-shadow-[0_0_8px_rgba(109,33,79,0.35)]">
            Teotia Shopprix
          </span>
        </Link>
        ...
        <div ref={boxRef} className="hidden md:block flex-1 max-w-2xl relative">
          <form onSubmit={handleSearch} className="flex rounded-full overflow-hidden border border-gold/40">
            <input
              ...
              className="flex-1 px-4 py-2 bg-luxe-panel text-slate-900 text-sm focus:outline-none placeholder:text-gray-500"
            />
        ...
        <div className="ml-auto flex items-center gap-4">
          ...
          {user && (
            <Link
              to="/wishlist"
              className="hidden sm:flex w-9 h-9 rounded-full bg-gold items-center justify-center text-white hover:bg-gold hover:text-white transition-colors"
              aria-label="Wishlist"
            >
              <Heart size={18} />
            </Link>
      ...
      <form onSubmit={handleSearch} className="md:hidden flex px-3 pb-3">
        <input
          ...
          onChange={(e) => handleKeywordChange(e.target.value)}
          placeholder="Search Teotia Shopprix"
          className="flex-1 px-4 py-2.5 bg-luxe-panel border border-gold/30 rounded-l-full text-slate-900 text-sm focus:outline-none placeholder:text-gray-500"
        />
