  ...
  return (
    ...
    >
      ...
      <button
        ...
        disabled={wishBusy}
        aria-label="Wishlist"
        className="absolute top-2 right-2 z-10 w-8 h-8 rounded-full bg-gold/50 flex items-center justify-center hover:scale-110 transition-transform disabled:opacity-60"
      >
      ...
      <div className="flex items-center gap-1 mb-1">
        <div className="flex text-gold">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star key={i} size={13} fill={i < Math.round(rating) ? '#6D214F' : 'none'} strokeWidth={1} />
          ))}
        </div>
      ...
      <button
        ...
        className="mt-auto bg-gold hover:bg-gold-light hover:scale-[1.02] text-white text-sm font-semibold py-2.5 rounded-full disabled:opacity-60 disabled:hover:scale-100 transition-all"
      >
